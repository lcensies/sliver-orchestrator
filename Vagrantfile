Vagrant.configure("2") do |config|
  ...
  # ---------------------------------------------------------
  # ১. Linux Victim: Pivot Host (Ubuntu)
  # ---------------------------------------------------------
  config.vm.define "linux_pivot" do |linux|
    linux.vm.box = "ubuntu/jammy64"

    # public_network এর বদলে private_network দিন যাতে কালি মেশিন একে চিনতে পারে
    # আইপিটি এমন দিন যা আপনার কালির সাবনেটে নেই (যেমন: 192.168.100.10)
    linux.vm.network "private_network", ip: "192.168.100.10"

    # Internal Network: sliver-lab (উইন্ডোজের সাথে কথা বলার জন্য)
  ...
  # ---------------------------------------------------------
  # ২. Windows Victim: Target Host
  # ---------------------------------------------------------
  config.vm.define "win_target" do |win|
    win.vm.box = "win10-local"
    
    # Internal Network: sliver-lab (শুধুমাত্র লিনাক্স একে চিনবে)
